...
//            exercises.let {
    ...
    override fun onCompletedChosen(plannedWorkout: PlannedWorkout, position: Int) {
        context?.let { context ->
            if (presenter.canBeCompleted(presenter.plannedWorkouts[position])) {
                ...
                (activity as MainActivity).showToast(R.string.cantBeCompleted)
            }
        }
    }
    ...
    override fun onWorkoutSelected(workout: Workout, date: Date) {
        context?.let { context ->
            ...
            if (date.after(Date())) {
                title = resources.getString(R.string.planWorkout)
                val text = resources.getString(R.string.planningFromCalendar)
                subtitle = "$text ${formatter.format(date)}"
                plannedWorkout.completed = false

            } else {
                title = resources.getString(R.string.logWorkout)
                subtitle = resources.getString(R.string.logWorkoutSubtitle)
