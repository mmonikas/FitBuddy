...
class ExercisesListPresenter {
    ...
    fun getOptionsForUpdatesListener(): FirestoreRecyclerOptions<Exercise>? {
        ...
        return if (user != null) {
            val query : Query = DatabaseService.getQueryForFetching(UserDataType.EXERCISE)
    ...
    fun removeItemAt(position: Int, completion: (result: FirebaseRequestResult) -> Unit) {
        ...
        DatabaseService.removeDocument(item, UserDataType.EXERCISE) {
            result ->
    ...
    fun checkIfExerciseCanBeDeleted(position: Int, completion: (result: Boolean?) -> Unit) {
        ...
        if (exerciseId != null) {
            DatabaseService.checkIfExistsInAnyWorkout(exerciseDocReference = exerciseId) {
                    existsSomewhere ->
